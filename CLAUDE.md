...
Phase 0 (Foundation) — complete. Phase 1 (Database & Auth) — complete. Phase 2 (Auth Flow & Route Guards) — complete. Phase 3 (Timeline Day View) — complete. Phase 4 (Add Photo Memory + R2 Storage) — complete. Phase 5 (Letter Memories) — complete. Phase 6 (Memory Detail & Actions) — complete. Phase 7 (Search and Tags) — complete. Phase 8 (Realtime Sync) — complete. Phase 9 (Collections) — complete. Phase 10A (Video & Ticket baseline) — complete. Phase 11 (Places Lens + coordinates: Google Places autocomplete picker, derived `/places` lens, lat/lng capture) — complete. Phase 11B (Places map via `react-native-maps`, Apple Maps on iOS — no key needed) — code-complete, pending dev-build verification (iOS only; Android not targeted). Phase 10B (Heavy video — SQLite upload queue, multipart R2 upload, queued card UI) — implemented on top of Phase 11, pending ship (native background-upload task, R2 AbortIncompleteMultipartUpload lifecycle rule, on-device verification). Places × heavy-video integration: a location picked on a heavy video is carried through the SQLite queue (`place_name`/`latitude`/`longitude` columns) so the queue processor persists it on the deferred memory insert. Deferred: place faves/cities chips, recent-places, `place:` search operator, place_name normalization, map clustering (see plan). Phase 13 (Timeline Zoom & Motion — Day/Month/Year zoom levels, segmented zoom bar, pinch gesture, FadeIn motion, anniversary/trip marker data wiring) — code-complete, pending on-device verification.
